...
percentage_by_utm <- clicks_by_utm %>% group_by(utm_source) %>% mutate(percentage = count/sum(count)) %>% filter(ad_clicked == TRUE)
percentage_by_utm
```
...
clicks_by_experimental <- ad_clicks %>% group_by(ad_clicked, experimental_group) %>% summarize(count=n())
clicks_by_experimental
```
